...
export default async (action: boolean) => {
    ...
    const worldPath = path.resolve(process.cwd(), '..', '..', 'minecraftWorlds');
    ...
    if (action) {
        ...
        console.log(`${chalk.green('✔')} ${chalk.bold('Behavior pack imported successfully!')}`)
    } else {
        ...
        if (file.find(item => item.pack_id === manifest.header.uuid)) {
            console.log(`${chalk.red('✘')} ${chalk.bold('Resource pack already exists in world resource packs!')}`)
            return;
        }
        ...
        console.log(`${chalk.green('✔')} ${chalk.bold('Resource pack imported successfully!')}`)
    }
}